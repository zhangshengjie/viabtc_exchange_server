...
def Conn():
    ...
    return MySQLdb.connect(host='testweb.com',user='root',passwd='123456',db='jinglanex',port=3306, charset="utf8")
	
def update_huobi():
    host = 'http://testweb.com/api/robot/robot-list'
    try:
...
def lastUpdateTime(cur):
    ...
    cur.execute('SELECT b.access_token,a.* FROM `jl_robot` a INNER JOIN `jl_api_access_token` b ON a.uid = b.`user_id` where a.market_id in (%s) and a.status = 1 LIMIT 10' % (marketids))
    data = cur.fetchall()
    print data
    if data:
        for info in data:
            ...
            url='http://testweb.com/api/exchange/order-limit'
