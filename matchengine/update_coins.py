...
def Check(cur,conn):
        ...
        markets_count = cur.execute("select stock,money from jl_exchange_coins where enable = 1 and stock in (%s) and money in (%s) order by listorder desc" % (assetss,assetss))
        ...
        if markets != conf_market_name:
                ...
                for x in old_markets:
                        new_market.append({'money':{'name':x[1],'prec':8},'name':x[0]+x[1],'min_amount':'0.001','stock':{'name':x[0],'prec':8}})
